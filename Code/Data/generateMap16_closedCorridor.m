function [map] = generateMap16_closedCorridor(config,camera)
...
%   1.2. Entities
nEntities = 4;
entityTypes = {'plane','plane','plane','plane'}';
%ground plane
...
plane4Parameters = [0,1,0,length/10]';
...
iEndPoints = iRightPoints(end)+1:iRightPoints(end)+nEndPoints;
nPoints = nGroundPoints + nLeftPoints + nRightPoints + nEndPoints;
%positions
...
endPositions = [width*rand(1,nEndPoints); length/10*ones(1,nEndPoints); 10*rand(1,nEndPoints)] + [-width/2 0 0]'*ones(1,nEndPoints);  
pointPositions = [groundPositions leftPositions rightPositions endPositions];
...
entityPoints = {[iGroundPoints]',[iLeftPoints]',[iRightPoints]',[iEndPoints]'}';
entityChildEntities = {[]',[]',[]',[]'}';
